        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('modules.update', $module->id) }}">
                ...
                <div class="mb-4">
                    ...
                    <input
                        class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
                        id="semester" type="number" name="semester" value="{{ old('semester', $module->semester) }}"
                        required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input
                        class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
                        id="credit" type="number" step="0.1" name="credit"
                        value="{{ old('credit', $module->credit) }}" required>
                </div>
                <div class="mb-4">
                    ...
                    <select
                        ...
                        <option value="draft" {{ old('status', $module->status) == 'draft' ? 'selected' : '' }}>
                            {{ __('Draft') }}</option>
                        <option value="publish" {{ old('status', $module->status) == 'publish' ? 'selected' : '' }}>
                            {{ __('Publish') }}</option>
                    </select>
                </div>
                <div class="mb-4">
                  ...
                  <select
                    ...
                    <option value="core" {{ old('category', $module->category) == 'core' ? 'selected' : '' }}>
                      {{ __('Core') }}</option>
                    <option value="elective" {{ old('category', $module->category) == 'elective' ? 'selected' : '' }}>
                      {{ __('Elective') }}</option>
                  </select>
                </div>
                <div class="mb-4">
